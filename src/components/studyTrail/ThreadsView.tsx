  ...
  return (
    <div style={{
      ...
      >
        ...
        {tagged ? <Tag size={13} style={{ flexShrink: 0, color: accent }} /> : <Waypoints size={13} style={{ flexShrink: 0, opacity: 0.7 }} />}
        <span style={{ minWidth: 0 }}>
          <span style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'rgb(var(--color-text-primary))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {thread.label}
          </span>
          {/* Says WHERE the topic came from, so a traced cluster is never mistaken for something
              Berean decided on its own authority. */}
          <span style={{ display: 'block', fontSize: 9.5, letterSpacing: '.03em', textTransform: 'uppercase', color: 'rgb(var(--color-text-muted))' }}>
            {thread.source}
          </span>
        </span>
        <span style={{ flex: 1 }} />
        ...
        <div style={{ padding: '0 12px 10px 36px' }}>
          ...
          {thread.terms.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 9.5, textTransform: 'uppercase', letterSpacing: '.04em', color: 'rgb(var(--color-text-muted))', marginBottom: 3 }}>Your words</div>
          ...
          {thread.chapters.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 9.5, textTransform: 'uppercase', letterSpacing: '.04em', color: 'rgb(var(--color-text-muted))', marginBottom: 3 }}>Passages</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        fontSize: 10.5, padding: '1px 7px', borderRadius: 999, cursor: 'pointer',
                        background: 'rgb(var(--color-surface-3))', border: 'none', color: 'rgb(var(--color-text-secondary))',
                      }}
                    ><BookOpen size={9} style={{ opacity: 0.6 }} />{c.label}</button>
                  )
                })}
              </div>
            </div>
          ...
          {thread.strongs.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 9.5, textTransform: 'uppercase', letterSpacing: '.04em', color: 'rgb(var(--color-text-muted))', marginBottom: 3 }}>Words looked up</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                {thread.strongs.map((sn) => (
                  ...
                  ><Hash size={9} style={{ opacity: 0.6 }} />{sn}</button>
                ))}
              </div>
            </div>
          )}

          <div style={{ fontSize: 9.5, textTransform: 'uppercase', letterSpacing: '.04em', color: 'rgb(var(--color-text-muted))', marginBottom: 3 }}>Sessions</div>
